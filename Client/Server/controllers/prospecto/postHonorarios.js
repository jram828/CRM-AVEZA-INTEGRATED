...
const guardaHonorarios = async (
  ...
  if (typeof valorRadicar === "number") {
    vRadicar = valorRadicar;
  } else {
    vRadicar = BigInt(valorRadicar);
  }
  ...
  if (typeof saldoHonorarios === "number") {
    sHonorarios = saldoHonorarios;
  } else {
    sHonorarios = BigInt(saldoHonorarios);
  }

  var sHonorariosUnificado;

  if (typeof saldoHonorariosUnificado === "number") {
    sHonorariosUnificado = saldoHonorariosUnificado;
  } else {
    sHonorariosUnificado = BigInt(saldoHonorariosUnificado);
  }
  ...
  if (deleteDeudas) {
    console.log(
      "deleteDeudas:",
      deleteDeudas,
      "-> eliminando asociaciones de honorarios para idProspecto:",
      idProspecto,
    );
    try {
      ...
      console.log(
        "Asociaciones de honorarios eliminadas para el prospecto:",
        idProspecto,
      );
      ...
      try {
        ...
        console.warn(
          "No se pudo obtener honorarios desde la instancia; se intentará con una consulta directa.",
          e,
        );
      }
      ...
      if (honorariosVinculados && honorariosVinculados.length > 0) {
        console.log(
          "Honorarios vinculados encontrados para evaluar eliminación:",
          honorariosVinculados.map((h) => h.idHonorario ?? h.id),
        );
        for (const h of honorariosVinculados) {
          try {
            ...
            const asociatedCount =
              typeof h.countProspectos === "function"
                ? await h.countProspectos()
                : 0;
            if (asociatedCount <= 1) {
              await h.destroy();
              console.log(
                "Honorario eliminado (no compartido):",
                h.idHonorario ?? h.id,
              );
            } else {
              console.log(
                "Honorario compartido con otros prospectos; dejado sin eliminar:",
                h.idHonorario ?? h.id,
              );
            }
          } catch (errDestroy) {
            console.error(
              "Error eliminando honorario:",
              h.idHonorario ?? h.id,
              errDestroy,
            );
          }
        ...
        console.log(
          "No se encontraron honorarios vinculados para eliminar para el prospecto:",
          idProspecto,
        );
      }
    } catch (err) {
    ...
    console.log(
      "deleteDeudas está en false: se mantienen los honorarios existentes",
    );
  }
  ...
  const newHonorario = await Honorario.create({
    ...
    saldoHonorarios: sHonorarios,
    saldoHonorariosUnificado: sHonorariosUnificado,
    // planpagos: planpagos,
