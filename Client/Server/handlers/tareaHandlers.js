...
const postCreateTareaHandler = async (req, res) =>{
    ...
    const { idProspecto, asunto, fechaVencimiento, recordatorio, tipoRecordatorio,repetir, frecuencia, repeticiones } = req.body
    ...
    try {
        const response = await createTarea(idProspecto, asunto, fechaVencimiento, recordatorio, tipoRecordatorio,repetir, frecuencia, repeticiones, email)
        res.status(200).json(response)
    } catch (error) {
        res.status(400).json({error:error.message})
    }
