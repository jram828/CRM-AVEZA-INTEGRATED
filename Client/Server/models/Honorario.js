import { DataTypes } from "sequelize";
import { v4 as uuidv4 } from "uuid";

export default (sequelize) => {
  const Honorario = sequelize.define(
    "Honorario",
    {
      idHonorario: {
        type: DataTypes.UUID,
        defaultValue: uuidv4,
        primaryKey: true,
      },
      valorHonorarios: { type: DataTypes.BIGINT, allowNull: false },
      honorarios_letras: { type: DataTypes.STRING, allowNull: true },
      valorRadicar: { type: DataTypes.BIGINT, allowNull: false },
      inicial: { type: DataTypes.BIGINT, allowNull: false },
      cuotasHonorarios: { type: DataTypes.INTEGER, allowNull: false },
      honorariosLiquidacion: { type: DataTypes.BIGINT, allowNull: true },
      honorariosLiquidacion_letras: { type: DataTypes.STRING, allowNull: true },
      saldoHonorarios: { type: DataTypes.BIGINT, allowNull: true },
      saldoHonorariosUnificado: { type: DataTypes.BIGINT, allowNull: true },
      totalDeudas: { type: DataTypes.BIGINT, allowNull: false },
    },
    {
      indexes: [
        {
          unique: true,
          fields: ["idHonorario"],
        },
      ],
    },
    { timestamps: false },
  );
  return Honorario;
};
