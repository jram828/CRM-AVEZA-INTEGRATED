...
const CitaFormDetail = ({open, onClose, onSave, selected, source}) => {
 ...
 if (source === "prospecto"){
    if (!selected) return;
    onSave({ ...dataCita, idProspecto: selected.idProspecto, source: "prospecto" });
    onClose();

  } else if (source === "cliente") {
    if (!selected) return;
    onSave({ ...dataCita, cedulaCliente: selected.cedulaCliente, source: "cliente" });
    onClose();
  }
};
